...
public class PowerUpLinha extends Peca {
	 ...
	 */
	public PowerUpLinha(int tp, int x, int y, float cX, float cY) {
		super(tp, 1, x, y, EngineConfig.INITIAL_POSITION,
				EngineConfig.INITIAL_POSITION, cX, cY, true);
	}

	// METODOS
	/**
	 * Remove da grelha a linha onde pertence a peca e coloca as pecas no array
	 * de pecas a Remover do ecra
	 * 
	 * @param grid
	 *            grelha de jogo
	 * @param pecasToRemove
	...
	public void destroiPeca(Peca[][] grid, ArrayList<Peca> pecasToRemove) {
		...
		if (grid[x][line] != null) {
			grid[x][line].setMoving(true);
			pecasToRemove.add(grid[x][line]);
			grid[x][line] = null;
		}
